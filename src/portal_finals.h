    ...
    std::shared_ptr<PortalStmt> start(const std::shared_ptr<Plan> &plan, Context *context)
    {
        ...
        else if (auto x = std::dynamic_pointer_cast<DMLPlan>(plan))
        {
            ...
            case T_Update:
            {
                ...
                std::unique_ptr<AbstractExecutor> root = std::make_unique<UpdateExecutor>(sm_manager_, x->tab_name_, x->set_clauses_, rids, context);
            ...
            case T_Delete:
            {
                ...
                std::unique_ptr<AbstractExecutor> root = std::make_unique<DeleteExecutor>(sm_manager_, x->tab_name_, rids, context);
