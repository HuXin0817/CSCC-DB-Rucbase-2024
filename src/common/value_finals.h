...
private:
    ...
    static std::unordered_map<std::string, int> name2fd;
};
...
struct TabMeta
{
    ...
    std::unordered_map<std::string, ColMeta> cols_idx_;
    std::vector<IndexMeta> indexes; 
    std::unordered_map<std::string, IndexMeta> indexes_idx_;
...
public:
    std::string name_;                                               
    std::unordered_map<std::string, std::unique_ptr<TabMeta>> tabs_; 
