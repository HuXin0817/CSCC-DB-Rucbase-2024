...
public:
    SortExecutor(std::unique_ptr<AbstractExecutor> prev, const TabCol &sel_col) : prev_(std::move(prev)), current_index(0)
    {
...
private:
    ...
    static Value getValue(const std::unique_ptr<RmRecord> &record, size_t offset, ColType col_type)
    {
