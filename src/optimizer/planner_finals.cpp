...
IndexMeta Planner::get_index_cols(const std::string &tab_name, const std::vector<Condition> &curr_conds)
{
    ...
    for (size_t idx_number_ = 0; idx_number_ < tab_->indexes.size(); idx_number_++)
    {
        int match_col_num = 0;
        // 遍历索引的列
        for (int i = 0; i < tab_->indexes[idx_number_].cols_.size(); i++)
        {
            // 如果当前索引列在条件列集合中
            if (conds_cols_.count(tab_->indexes[idx_number_].cols_[i].name))
            {
                match_col_num++;
            }
            else
    ...
    if (ans)
    {
        return tab_->indexes[matched_index_number_];
    }
    return {};
}
...
bool Planner::get_merge_join_index(const std::string &tab_name, const TabCol &col)
{
    ...
    {
        if (index.cols_.front().name == col.col_name)
        {
            return true;
        }
    }
...
std::shared_ptr<Plan> pop_scan(int *scantbl, const std::string &table, std::vector<std::string> &joined_tables, std::vector<std::shared_ptr<Plan>> plans)
{
...
std::shared_ptr<Plan> Planner::make_one_rel(const std::shared_ptr<Query> &query, Context *context)
{
    ...
    for (size_t i = 0; i < tables.size(); i++)
    {
        ...
        { // 该表没有索引
            table_scan_executors[i] = std::make_shared<ScanPlan>(T_SeqScan, sm_manager_, tables[i], curr_conds, index_meta);
        }
        else
        { // 存在索引
            table_scan_executors[i] = std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, tables[i], curr_conds, index_meta);
        }
    }
    ...
    auto scantbl = new int[tables.size()];
    for (size_t i = 0; i < tables.size(); i++)
    {
        scantbl[i] = -1;
    }
...
Planner::generate_join_sort_plan(const std::string &table, std::vector<Condition> &conds, TabCol &col, std::shared_ptr<Plan> plan)
{
    ...
    for (auto &cond : conds)
    {
        ...
        else // cond.rhs_col.tab_name == table
        {
            col = cond.rhs_col;
            cond.join_cond = true;
            break;
    ...
    if (exist_index)
    {
        ...
        return std::make_shared<ScanPlan>(T_IndexScan, sm_manager_, scan_plan->tab_name_, scan_plan->conds_,
                                          index_col_names);
    }
    // 否则，返回排序计划
